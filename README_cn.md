# KOL Insight - Twitter KOL 评价系统

一个完整的 Twitter KOL（关键意见领袖）评价系统，支持自动爬取 Twitter 数据、AI 评分分析和生成评价网页。
...
编辑 `generate_report.py`，修改第 27 行的用户名：
...
│
├── data/                       # 数据目录
│   └── twitter_data_*.db       # 爬取的原始数据（SQLite 数据库）
│
├── outputs/                    # 输出目录
...
在 `generate_report.py` 或 `app.py` 中可以修改以下配置：
...
**重要提示**：`outputs/tree_structure_*.json` 文件是根据 `schema.py` 自动生成的，不要直接修改该文件。
